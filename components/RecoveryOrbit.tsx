...
type Props = {
  ...
  /** When set (e.g. a Stories "View in visualization" action), every country
   * in this list is emphasized and labeled together, and everything else is
   * dimmed — instead of only ever emphasizing one country at a time. */
...
export default function RecoveryOrbit({
  ...
  useEffect(() => {
    ...
    const spotlightSet = new Set(spotlightIso3s ?? []);
    const isSpotlightActive = spotlightSet.size > 0;
    const isEmphasized = (iso3: string) =>
      isSpotlightActive ? spotlightSet.has(iso3) : iso3 === activeIso3 || iso3 === compareIso3;
    const isDimmed = (iso3: string) => {
      if (isSpotlightActive) return !spotlightSet.has(iso3);
      if (!activeIso3 && !compareIso3) return false;
      return iso3 !== activeIso3 && iso3 !== compareIso3;
    };

    const focusPoints = plotted.filter(
      (p) => spotlightSet.has(p.country.iso3) || [activeIso3, compareIso3].includes(p.country.iso3),
    );
    ...
    const currentDots = plotted.map((p) => {
      ...
      const isSpotlighted = spotlightSet.has(p.country.iso3);
      const isPrimary = isSelected || isCompare;
      const showLabel = isPrimary || isHovered || isSpotlighted;
      return {
        ...
        itemStyle: {
          color: REGION_COLORS[p.country.region] ?? "#aab6c5",
          borderColor: isPrimary ? "#ffffff" : isSpotlighted ? "rgba(255,255,255,.65)" : "#1b212b",
          borderWidth: isPrimary ? 3.5 : isSpotlighted ? 2.4 : 1.5,
          opacity: isDimmed(p.country.iso3) ? 0.18 : 0.9,
          shadowBlur: isPrimary || isHovered || isSpotlighted ? 18 : 0,
  ...
  }, [plotted, lens, selectedIso3, hoveredIso3, compareIso3, activeIso3, spotlightIso3s]);
    ...
    <div className="orbit-wrap">
      ...
      <p className="chart-caption">
        ...
        {spotlightIso3s && spotlightIso3s.length > 0 ? (
          <span>Highlighting the {spotlightIso3s.length} countries from this story — select any other country to clear it.</span>
        ) : (
